<template>
  <div>
    ...
    <div v-if="canManageUsers && !loading" class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      <!-- Total Users Card -->
      <div
        class="relative bg-gradient-to-br from-primary/10 to-primary/5 rounded-2xl p-6 border border-primary/20 overflow-hidden group hover:shadow-lg transition-all duration-300">
        <div
      ...
      <!-- Admins Card -->
      <div
        class="relative bg-gradient-to-br from-error/10 to-error/5 rounded-2xl p-6 border border-error/20 overflow-hidden group hover:shadow-lg transition-all duration-300">
        <div
          class="absolute top-0 right-0 w-32 h-32 bg-error/10 rounded-full -mr-16 -mt-16 group-hover:scale-110 transition-transform duration-500">
        </div>
        <div class="relative flex items-start justify-between">
          <div>
            <p class="text-sm font-medium text-base-content/60 mb-2">Admins</p>
            <h3 class="text-4xl font-bold text-error mb-1">{{users.filter((u: { role: string; }) => u.role ===
              'admin').length}}</h3>
          ...
          <div class="w-12 h-12 bg-error/20 rounded-xl flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="w-6 h-6 stroke-error"
              stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round"
      ...
      <!-- Moderators Card -->
      <div
        class="relative bg-gradient-to-br from-warning/10 to-warning/5 rounded-2xl p-6 border border-warning/20 overflow-hidden group hover:shadow-lg transition-all duration-300">
        <div
          class="absolute top-0 right-0 w-32 h-32 bg-warning/10 rounded-full -mr-16 -mt-16 group-hover:scale-110 transition-transform duration-500">
        </div>
        <div class="relative flex items-start justify-between">
          <div>
            <p class="text-sm font-medium text-base-content/60 mb-2">Moderators</p>
            <h3 class="text-4xl font-bold text-warning mb-1">{{users.filter((u: { role: string; }) => u.role ===
              'moderator').length}}</h3>
    ...
    <div v-if="canManageUsers" class="bg-base-100 rounded-2xl shadow-xl border border-base-300/50 overflow-hidden">
      <div class="p-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h2
              ...
              All Users</h2>
            <p class="text-sm text-base-content/60">View and manage all user accounts</p>
          </div>
        ...
        <div class="flex flex-col sm:flex-row gap-4 mb-6">
          <div class="flex-1">
            <UiInput v-model="searchQuery" placeholder="Search by name or email..." class="w-full" />
          </div>
          <UiSelect v-model="roleFilter" class="w-full sm:w-48">
        ...
        <!-- Users Table -->
        <div v-else-if="filteredUsers.length > 0" class="overflow-x-auto -mx-8">
          <table class="table">
            <thead>
              ...
              <tr v-for="u in filteredUsers" :key="u.id"
                ...
                <td>
                  <div class="flex items-center gap-3">
                    <div class="avatar placeholder">
                      <div
                        class="bg-gradient-to-br from-primary to-secondary text-primary-content rounded-full w-10 shadow-md flex items-center justify-center">
                ...
                <td v-if="user?.role === 'admin'" class="text-right">
                  <div class="inline-flex gap-2">
                    <button @click="editUser(u)" class="btn btn-sm btn-ghost hover:btn-primary group" title="Edit user">
                    ...
                    <button @click="deleteUser(u.id)" :disabled="u.id === user?.id"
                      class="btn btn-sm btn-ghost hover:btn-error group disabled:opacity-30" title="Delete user">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 group-hover:scale-110 transition-transform"
                        fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div v-else class="bg-base-100 rounded-2xl shadow-xl border border-base-300/50 overflow-hidden">
      <div class="text-center py-20 px-8">
        <div
          class="w-24 h-24 mx-auto mb-6 bg-gradient-to-br from-error/20 to-warning/20 rounded-full flex items-center justify-center">
        ...
        <p class="text-base-content/60 mb-8 max-w-md mx-auto">You don't have permission to view users. Contact your
          ...
          <div v-if="showCreateModal || showEditModal"
            ...
            <div class="relative bg-gradient-to-br from-primary to-secondary p-8 text-primary-content overflow-hidden">
              ...
              <div class="relative flex items-start gap-4">
                <div
                  class="shrink-0 w-14 h-14 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center shadow-lg">
                ...
                <div class="flex-1">
                  <h3 class="text-2xl font-bold mb-1">{{ modalTitle }}</h3>
                  <p class="text-sm opacity-90">{{ modalDescription }}</p>
                </div>
            ...
            <form @submit.prevent="showEditModal ? handleUpdate() : handleCreate()" class="p-8 space-y-6">
              <!-- Name Field -->
              <UiInput v-model="formData.name" label="Full Name" placeholder="Enter full name" required :icon="true">
                <template #icon>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
              ...
              <!-- Email Field -->
              <UiInput v-model="formData.email" type="email" label="Email Address" placeholder="user@example.com"
                required :icon="true">
                <template #icon>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
              ...
              <!-- Password Field -->
              <UiInput v-model="formData.password" type="password" label="Password"
                :placeholder="showEditModal ? 'Leave blank to keep current' : 'Minimum 6 characters'"
                :required="!showEditModal" minlength="6" :icon="true"
                :helper-text="showEditModal ? 'Leave blank to keep current password' : 'Minimum 6 characters required'">
                <template #icon>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <!-- Role Field -->
              <UiSelect v-model="formData.role" label="User Role" required
              ...
              <div class="flex gap-3 pt-4">
                <button type="button" @click="closeModal" class="btn btn-ghost flex-1 h-12 hover:bg-base-200">
                  Cancel
                </button>
...
const modalTitle = computed(() => {
  return showEditModal.value ? 'Edit User' : 'Create New User';
});
